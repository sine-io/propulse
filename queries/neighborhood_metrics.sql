...
SELECT
  ...
  COUNT(*) FILTER (WHERE listing_snapshots.layout = sqlc.arg(target_layout))::int AS target_layout_supply
FROM listing_snapshots
WHERE listing_snapshots.neighborhood_id = sqlc.arg(neighborhood_id)
...
WHERE neighborhood_id = sqlc.arg(neighborhood_id)
  ...
  AND coverage = 'full'
  AND (collected_at, id) <= (sqlc.arg(trigger_collected_at), sqlc.arg(trigger_run_id)::uuid)
ORDER BY collected_at DESC, id DESC
LIMIT 1;
...
-- name: AggregateMarketObservations :one
WITH trigger_run AS (
  SELECT id, data_source_id, neighborhood_id, collected_at, coverage
  FROM collection_runs
  WHERE collection_runs.id = sqlc.arg(trigger_run_id)
    AND collection_runs.neighborhood_id = sqlc.arg(neighborhood_id)
...
-- name: UpsertNeighborhoodMetric :one
INSERT INTO neighborhood_metrics (
  neighborhood_id,
  listed_homes,
  price_cut_homes,
