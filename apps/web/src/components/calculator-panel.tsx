...
export function CalculatorPanel() {
  ...
  const [oldChoice, setOldChoice] = useState<"" | "none" | string>("");
  const [oldSalePrice, setOldSalePrice] = useState("");
  const [oldLoanBalance, setOldLoanBalance] = useState("");
  const [oldPriceConfirmed, setOldPriceConfirmed] = useState(false);
  ...
  const selectedNeighborhood = neighborhoods.find((item) => item.id === targetNeighborhoodID);
  ...
  useEffect(() => {
    ...
    searchNeighborhoods({ q: neighborhoodQuery, page: 1, pageSize: 100 }, controller.signal)
    ...
    return () => controller.abort();
    // Searches are committed with the search button.
  ...
  useEffect(() => {
    if (!targetNeighborhoodID || !targetRoomID) {
      setTargetListing(undefined);
      return;
    }
    const controller = new AbortController();
    getMarketListingDetail(targetNeighborhoodID, targetRoomID, controller.signal)
      .then((detail) => {
    ...
    return () => controller.abort();
    // invalidateReport is intentionally local state invalidation, not an effect dependency.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [targetNeighborhoodID, targetRoomID]);
  ...
  const chooseNeighborhood = (item: Neighborhood) => {
    setTargetNeighborhoodID(item.id);
    setTargetRoomID("");
    setTargetListing(undefined);
    setTargetPrice("");
    setTargetPriceConfirmed(false);
  ...
  const chooseListing = (listing: MarketListing) => {
    setTargetRoomID(listing.roomId);
    setTargetPrice(String(listing.listingTotalPriceWan));
    setTargetPriceConfirmed(false);
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (requestVersion.current === version) setReport(response);
    } catch (error) {
  ...
  return (
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
      <section className="border border-slate-200 bg-white shadow-sm lg:col-span-5">
        <form onSubmit={submit} noValidate>
          <StepSection number="1" title="选择旧房资产">
            <div className="grid gap-2">
              <ChoiceButton selected={oldChoice === "none"} onClick={() => chooseOldHome("none")} icon={Home} title="无旧房" detail="本次测算不计旧房回款" />
              {accessState === "locked" ? <StatusBand tone="amber">个人资产已锁定，仍可选择无旧房路径。</StatusBand> : null}
              ...
              {assets.map((asset) => (
                <ChoiceButton key={asset.id} selected={oldChoice === asset.id} onClick={() => chooseOldHome(asset)} icon={Building2} title={asset.name} detail={`${asset.property.layout} · ${formatNumber(asset.property.areaSqm)}㎡ · 贷款 ${formatNumber(asset.currentLoanBalanceWan)} 万`} />
              ))}
            </div>
            {fieldErrors.oldChoice ? <FieldError>{fieldErrors.oldChoice}</FieldError> : null}
            {selectedAsset ? (
              <div className="mt-4 grid gap-4 border-l-4 border-blue-500 bg-blue-50 p-4 sm:grid-cols-2">
                <NumberField label="旧房预期售价 (万)" value={oldSalePrice} error={fieldErrors.oldSalePrice} onChange={(value) => { setOldSalePrice(value); setOldPriceConfirmed(false); invalidateReport(); }} />
                <NumberField label="当前贷款余额 (万)" value={oldLoanBalance} error={fieldErrors.oldLoanBalance} onChange={(value) => { setOldLoanBalance(value); invalidateReport(); }} />
                <ConfirmPrice checked={oldPriceConfirmed} onChange={(checked) => { setOldPriceConfirmed(checked); invalidateReport(); }} label="确认采用该旧房预期售价" />
                <label className="flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={oldHomeOnlyFamilyHome} onChange={(event) => { setOldHomeOnlyFamilyHome(event.target.checked); invalidateReport(); }} className="h-4 w-4 accent-blue-600" />旧房是家庭唯一住房</label>
                <p className="sm:col-span-2 text-xs text-slate-500">参考挂牌 {selectedAsset.property.currentListingPriceWan == null ? "未记录" : `${formatNumber(selectedAsset.property.currentListingPriceWan)} 万`} · 资产确认于 {formatDateTime(selectedAsset.updatedAt)}</p>
              </div>
            ) : null}
          </StepSection>

          <StepSection number="2" title="选择目标房源">
            <div className="flex gap-2">
              <input aria-label="搜索目标小区" value={neighborhoodQuery} onChange={(event) => setNeighborhoodQuery(event.target.value)} className="h-10 min-w-0 flex-1 rounded-md border border-slate-300 px-3 text-sm outline-none focus:border-blue-500" />
              <button type="button" onClick={() => setNeighborhoodsVersion((value) => value + 1)} className="h-10 rounded-md bg-slate-900 px-3 text-sm font-medium text-white">搜索</button>
            </div>
            {neighborhoodsState === "loading" ? <InlineLoading label="正在读取小区" /> : null}
            {neighborhoodsState === "failed" ? <RetryState label="小区读取失败" onRetry={() => setNeighborhoodsVersion((value) => value + 1)} /> : null}
            {neighborhoodsState === "ready" ? (
              <div className="mt-3 flex max-h-36 flex-wrap gap-2 overflow-y-auto">
                {neighborhoods.map((item) => <button key={item.id} type="button" aria-pressed={targetNeighborhoodID === item.id} onClick={() => chooseNeighborhood(item)} className="rounded-md border border-slate-300 px-3 py-2 text-left text-sm text-slate-700 aria-pressed:border-blue-500 aria-pressed:bg-blue-50 aria-pressed:text-blue-900"><span className="font-medium">{item.name}</span><span className="ml-2 text-xs text-slate-500">{item.area}</span></button>)}
              </div>
            ) : null}
            {fieldErrors.targetNeighborhood ? <FieldError>{fieldErrors.targetNeighborhood}</FieldError> : null}

            {listingsState === "loading" ? <InlineLoading label="正在读取当前房源" /> : null}
            {listingsState === "failed" ? <RetryState label="当前房源读取失败" onRetry={() => setListingsVersion((value) => value + 1)} /> : null}
            {listingsState === "ready" && listings.length === 0 ? <StatusBand tone="amber">该小区暂无当前在售房源，请选择其他小区或刷新数据。</StatusBand> : null}
            {listingsState === "ready" && listings.length > 0 ? (
              <div className="mt-4 max-h-64 overflow-y-auto border-y border-slate-200">
                {listings.map((listing) => (
                  <label key={listing.roomId} className="flex cursor-pointer items-center gap-3 border-b border-slate-100 px-2 py-3 last:border-b-0 hover:bg-slate-50">
                    <input type="radio" name="target-listing" checked={targetRoomID === listing.roomId} onChange={() => chooseListing(listing)} className="h-4 w-4 flex-none accent-blue-600" />
            ...
            {targetListing ? (
              <div className="mt-4 grid gap-4 border-l-4 border-emerald-500 bg-emerald-50 p-4 sm:grid-cols-2">
                <NumberField label="预计成交价 (万)" value={targetPrice} error={fieldErrors.targetPrice} onChange={(value) => { setTargetPrice(value); setTargetPriceConfirmed(false); invalidateReport(); }} />
                ...
                <ConfirmPrice checked={targetPriceConfirmed} onChange={(checked) => { setTargetPriceConfirmed(checked); invalidateReport(); }} label="确认采用该目标房成交价" />
          ...
          <StepSection number="3" title="确认家庭资金">
            ...
            <div className="mt-5">
              ...
              <div className="inline-flex rounded-md border border-slate-300 p-1">
                <SegmentButton selected={homePurchaseOrder === "first"} onClick={() => { setHomePurchaseOrder("first"); invalidateReport(); }}>首套</SegmentButton>
                <SegmentButton selected={homePurchaseOrder === "second"} onClick={() => { setHomePurchaseOrder("second"); invalidateReport(); }}>二套</SegmentButton>
              </div>
            </div>
            ...
            <details className="mt-5 border-t border-slate-200 pt-4">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <SelectField label="贷款类型" value={loanType} onChange={(value) => { setLoanType(value as LoanPlan["type"]); invalidateReport(); }} options={[{ value: "commercial", label: "商业贷款" }, { value: "provident_fund", label: "公积金贷款" }, { value: "combined", label: "组合贷款" }]} />
                <SelectField label="贷款期限" value={String(loanTermMonths)} onChange={(value) => { setLoanTermMonths(Number(value)); invalidateReport(); }} options={[120, 180, 240, 300, 360].map((value) => ({ value: String(value), label: `${value / 12} 年` }))} />
                <SelectField label="还款方式" value={repaymentMethod} onChange={(value) => { setRepaymentMethod(value as LoanPlan["repaymentMethod"]); invalidateReport(); }} options={[{ value: "equal_installment", label: "等额本息" }, { value: "equal_principal", label: "等额本金" }]} />
                <SelectField label="税费承担" value={taxBurdenMode} onChange={(value) => { setTaxBurdenMode(value as "statutory" | "buyer_all"); invalidateReport(); }} options={[{ value: "statutory", label: "依法各自承担" }, { value: "buyer_all", label: "买方承担全部" }]} />
              </div>
              {loanType === "combined" ? (
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <NumberField label="贷款总额 (万)" value={combinedTotal} error={fieldErrors.combinedTotal} onChange={(value) => { setCombinedTotal(value); invalidateReport(); }} />
                  <NumberField label="商贷金额 (万)" value={commercialAmount} error={fieldErrors.commercialAmount} onChange={(value) => { setCommercialAmount(value); invalidateReport(); }} />
                  <NumberField label="公积金金额 (万)" value={providentAmount} error={fieldErrors.providentAmount} onChange={(value) => { setProvidentAmount(value); invalidateReport(); }} />
                </div>
              ) : null}
              ...
              <ManualOverridesEditor manual={manual} error={fieldErrors.manual} onChange={(value) => { setManual(value); invalidateReport(); }} />
            </details>
          </StepSection>

          <div className="p-5 pt-0 sm:p-6 sm:pt-0">
            <button type="submit" disabled={assumptionsState !== "ready" || isSubmitting} className="flex h-12 w-full items-center justify-center gap-2 rounded-md bg-blue-600 px-4 font-medium text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-slate-300">
      ...
      <section className="space-y-6 lg:col-span-7">
        <div className="min-h-[32rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
          <div className="mb-6 flex flex-wrap items-end justify-between gap-2 border-b border-slate-100 pb-4">
            ...
            <p className="text-sm text-slate-500">{report ? `记录 ${report.id}` : "等待确认"}</p>
          </div>
          {report ? <CalculationReport report={report} /> : <p className="border border-dashed border-slate-200 bg-slate-50 p-6 text-center text-sm text-slate-500">完成房屋选择、价格确认与家庭资金后生成报告。</p>}
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <MethodLink href="/methods/monthly-payment-safety" title="为什么月供安全线比总价更重要？" description="了解现金流断裂的风险" />
          <MethodLink href="/methods/old-home-sale-delay" title="旧房迟迟卖不掉怎么办？" description="学会测算底线成交价" />
        </div>
      </section>
    </div>
  );
...
function StepSection({ children, number, title }: { children: React.ReactNode; number: string; title: string }) {
  return <fieldset className="border-b border-slate-200 p-5 sm:p-6"><legend className="mb-5 flex items-center gap-3 text-base font-semibold text-slate-900"><span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-xs text-white">{number}</span>{title}</legend>{children}</fieldset>;
}

function ChoiceButton({ detail, icon: Icon, onClick, selected, title }: { detail: string; icon: typeof Home; onClick: () => void; selected: boolean; title: string }) {
  return <button type="button" aria-pressed={selected} onClick={onClick} className="grid min-h-14 grid-cols-[auto_1fr_auto] items-center gap-3 rounded-md border border-slate-200 px-3 py-2 text-left aria-pressed:border-blue-500 aria-pressed:bg-blue-50"><Icon aria-hidden="true" className="h-5 w-5 text-slate-500" /><span className="min-w-0"><span className="block break-words text-sm font-semibold text-slate-900">{title}</span><span className="block break-words text-xs text-slate-500">{detail}</span></span>{selected ? <Check aria-hidden="true" className="h-5 w-5 text-blue-700" /> : null}</button>;
}
...
function CalculationReport({ report }: { report: CalculationResponse }) {
  ...
  return <div className="space-y-8">
    <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs text-slate-500"><span>报告 ID：{report.id}</span><time dateTime={report.createdAt}>生成时间：{formatDateTime(report.createdAt)}</time><span>追溯状态：{result.traceabilityStatus}</span></div>
    {report.selectionContext ? <SelectionSnapshotReport context={report.selectionContext} /> : null}
    {applied ? <PressureScale ratio={result.monthlyPaymentRatio} thresholds={applied.pressureThresholds} pressure={pressure} targetTotalPrice={report.input.targetTotalPrice} /> : <p className="border-l-4 border-amber-400 bg-amber-50 p-4 text-sm text-amber-900">该历史记录未保存版本化假设，无法复现压力阈值。</p>}
...
function buildCalculationInput(args: {
  ...
  if (!args.oldChoice) errors.oldChoice = "请选择无旧房或一条资产";
  let oldHomeValue = 0;
